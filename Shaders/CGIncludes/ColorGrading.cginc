	...
	sampler2D blue)
{
	...
	// Y
	c += kHalfPixel.xxx;
	float mr = tex2D(master, float2(c.r, 0.5)).r;
	float mg = tex2D(master, float2(c.g, 0.5)).r;
	float mb = tex2D(master, float2(c.b, 0.5)).r;
	c = saturate(float3(mr, mg, mb));

	// RGB
	c += kHalfPixel.xxx;
	float r = tex2D(red, float2(c.r, 0.5)).r;
	float g = tex2D(green, float2(c.g, 0.5)).r;
	float b = tex2D(blue, float2(c.b, 0.5)).r;
...
{
	half offset = saturate(tex2D(curveTex, half2(hue, 0.5)).r) - 0.5;
	hue += offset;
...
{
	return saturate(tex2D(curveTex, half2(hue, 0.5)).r) * 2.0;
}
...
{
	return saturate(tex2D(curveTex, half2(sat, 0.5)).r) * 2.0;
}
...
{
	return saturate(tex2D(curveTex, half2(lum, 0.5)).r) * 2.0;
}
